...
public class PagedRecipeMenu extends PagedMenu {
    ...
    public PagedRecipeMenu(Player viewer, ItemStack itemStack) {
        ...
        recipes = Bukkit.getServer().getRecipesFor(itemStack).stream().filter(recipe -> recipe instanceof ShapedRecipe || recipe instanceof ShapelessRecipe).toList();
        initialize();
    }
    ...
    public void fillInventoryBorder() {
        ...
        setBorderBasedOfList(graySlots, Material.GRAY_STAINED_GLASS_PANE);
        setBorderBasedOfList(orangeSlots, Material.ORANGE_STAINED_GLASS_PANE);
        setBorderBasedOfList(brownSlots, Material.BROWN_STAINED_GLASS_PANE);
        setBorderBasedOfList(limeGreenSlots, Material.LIME_STAINED_GLASS_PANE);
        setBorderBasedOfList(greenSlots, Material.GREEN_STAINED_GLASS_PANE);
    }

    private void setBorderBasedOfList(List<Integer> slots, Material material) {
        for (int slot : slots) {
    ...
    private MenuButton getRecipePart(ItemStack itemStack) {
        ...
        return new MenuButton(itemStack).setClicker(player -> close());
    }
