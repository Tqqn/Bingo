...
public abstract class AbstractConfig {
    ...
    public AbstractConfig(String configName, AbstractDatabaseModule dataModule) {
    ...
    private void loadConfig() {
        file = new File(databaseModule.getModuleManager().getPlugin().getDataFolder(), configName);
        if (!file.exists()) {
            file.getParentFile().mkdirs();
            databaseModule.getModuleManager().getPlugin().saveResource(configName, false);
        }
    ...
    public void save() {
        Bukkit.getScheduler().runTaskAsynchronously(databaseModule.getModuleManager().getPlugin(), () -> {
            try {
    ...
    public void saveValue(String path, Object value, boolean async, boolean save) {
        if (async) {
            Bukkit.getScheduler().runTaskAsynchronously(databaseModule.getModuleManager().getPlugin(), () -> getConfiguration().set(path, value));
        }
        if (save) this.save();
    }
